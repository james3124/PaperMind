...
export default function CitationSheet({
  visible, initialStyle = 'apa', initialEdition = '7th', onConfirm, onDismiss,
}: Props) {
  ...
  const styleName = STYLES.find((s) => s.id === style)?.label ?? style.toUpperCase();
        ...
        <View style={styles.sheet}>
          ...
          <View style={styles.chipRow}>
            {STYLES.map((s) => (
              ...
              >
                <Text style={[styles.chipText, style === s.id && styles.chipTextSelected]}>
                  {s.label}
                </Text>
              </TouchableOpacity>
            ))}
            ...
            <>
              ...
              <View style={styles.chipRow}>
                {editions.map((ed) => (
                  ...
                  >
                    <Text style={[styles.chipText, edition === ed && styles.chipTextSelected]}>
                      {ed}
                    </Text>
                  </TouchableOpacity>
                ))}
...
const styles = StyleSheet.create({
  ...
  editionLabel:    { fontSize: 13, fontWeight: '600', color: '#374151', marginBottom: 8 },
  confirmButton:   { backgroundColor: '#6366f1', padding: 16, borderRadius: 12, alignItems: 'center', marginTop: 16 },
