...
import {copyBlankTemplate} from './paperFileStore';

// Papers generated by the pipeline keep their markdown here until the editor
// screen opens the document and inserts it (Task 11 wires insertMarkdown).
const pendingMarkdownByDoc = new Map<string, string>();
...
export async function* runPipeline(
  ...
  // ── Stage 19: Save to WatermelonDB + docx file ───────────────────────────────
  ...
  try {
    ...
    // repository.create() already provisions the blank docx; re-copying is
    // idempotent and keeps stage 19 self-sufficient.
    try {
      await copyBlankTemplate(doc.id);
    } catch {
      // Non-fatal: the template file already exists from create().
    }
    await documentRepository.update(doc.id, {
      content: `papers/${doc.id}.docx`,
      wordCount,
      status: 'aiReady',
    });
    // The editor screen inserts this markdown once the SuperDoc bridge is
    // ready (insertMarkdown lands in Task 11).
    pendingMarkdownByDoc.set(doc.id, fullPaperText);
    documentId = doc.id;
  } catch (e: unknown) {
    yield {
      type: 'error',
