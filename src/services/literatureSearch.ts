...
async function fetchOpenAlex(query: string): Promise<SourcePaper[]> {
  const res = await axios.get<{
    ...
    params: {
      ...
      'per-page': 20,
      select:
        'title,authorships,publication_year,abstract_inverted_index,doi,id',
...
async function fetchArXiv(query: string): Promise<SourcePaper[]> {
  const res = await axios.get<string>('https://export.arxiv.org/api/query', {
    params: {
...
function parseArXivXml(xml: string): SourcePaper[] {
  ...
  for (const entry of entryMatches) {
    ...
    const authorMatches =
      entry.match(/<author>[\s\S]*?<name>(.*?)<\/name>[\s\S]*?<\/author>/g) ??
      [];
