...
import ColorPaletteModal, {ColorKind} from '@/components/editor/ColorPaletteModal';
...
export default function EditorScreen({route, navigation}: Props) {
  ...
  const onFormatChange  = useCallback((f: Record<string, unknown>) => setFormat(f), []);
  const onHeadings      = useCallback((h: {level: number; text: string; index: number}[]) => setHeadings(h), []);
  const onReplaceResult = useCallback((count: number) => setReplaceCount(count), []);
  ...
  const handleAiAction = useCallback(async (prompt: string, text: string) => {
    if (!modelReady) {
      Alert.alert('Model not ready', 'The AI model is still downloading. Please wait.');
      return;
    }
    try {
  ...
  const handleInsertImage = useCallback(async () => {
    try {
      ...
      if (!DocumentPicker.isCancel(e)) {
        Alert.alert('Image failed', e instanceof Error ? e.message : 'Unknown error');
      }
    }
  }, []);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <TabToolbar
        ...
        onPaperSize={size => { setPaperSize(size); editorRef.current?.setPaperSize(size); }}
        onOpenColor={kind => setColorKind(kind)}
        onSpacing={value => editorRef.current?.format('spacing', value === false ? false : value)}
        ...
        <View style={styles.flex}>
          ...
          <EditorWebView
            ...
            onSelectionText={text => {
              if (text.trim()) { setSelectedText(text); setShowAi(true); }
      ...
      <ColorPaletteModal
        ...
        onSelect={hex => editorRef.current?.format(colorKind === 'background' ? 'background' : 'color', hex)}
        onClear={() => editorRef.current?.format(colorKind === 'background' ? 'background' : 'color', false)}
      ...
      return delta.ops
        .map((op: {insert: unknown}) => {
          ...
          if (embed && 'paper-table' in embed) {
            const table = (embed as {'paper-table': {html: string}})['paper-table'];
            return (table.html ?? '')
              .replace(/<t[dh][^>]*>/gi, ' ')
              .replace(/<\/t[dh]>/gi, '  ')
              .replace(/<[^>]+>/g, '')
          ...
          if (embed && 'page-break' in embed) return '\n\n';
          if (embed && 'image' in embed) return '[image] ';
          return '';
        })
        .join('');
    }
